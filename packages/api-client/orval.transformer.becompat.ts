...
// 리소스 세그먼트 단복수. 현 리소스가 전부 규칙적 복수(feeds, coworker-requests …)라
// naive `-s`/`+s` 로 충분 — 손-맵 불필요. 불규칙 리소스(예: people/children) 생기면
// 여기에만 예외 추가.
const singularize = (r: string): string => pascal(r.endsWith('s') ? r.slice(0, -1) : r)
