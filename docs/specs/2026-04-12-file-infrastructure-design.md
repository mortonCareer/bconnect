...
CREATE INDEX idx_attachments_path ON attachments (path) WHERE deleted_at IS NULL;
```

**`path` 필드 의미:** S3 key의 **디렉토리 prefix**만 저장. 같은 컨텍스트(예: `private/chats/42/images`)에 여러 첨부가 있으면 path 값이 동일함 (UNIQUE 제약 없음). 실제 S3 key는 FE/BE가 `path` + `id` + `size` + `ext`로 조합 (§6.4 참고).

**예시:**

- 채팅 이미지: `path = "private/chats/42/images"`, 실제 S3 key = `private/chats/42/images/o/{id}.{ext}`
- 자격증 파일: `path = "private/credentials/7/files"`, 실제 S3 key = `private/credentials/7/files/{id}.{ext}`
...
type AttachmentMeta = {
  ...
  path: string // 디렉토리 prefix (예: "private/chats/42/images")
...
**왜 `path`를 UNIQUE로 두지 않는가**: 같은 컨텍스트(예: 채팅방 42)의 여러 이미지가 모두 동일 `path = "private/chats/42/images"`를 공유. 유일성은 `id` (PK)로 보장.
