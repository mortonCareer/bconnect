...
> **You'll be able to**: production·dev·local 각 환경의 FE/BE 도메인, FE→BE 연결, 보조 서비스(DB·S3·FCM)·DNS 현황을 한눈에 확인

도메인의 **SSoT 는 [`infra/`](../../infra/) (Terraform)** 다. 이 문서는 그걸 보기 쉽게 옮겨 적은 **요약본** — 도메인이 바뀌면 `infra/` 가 먼저 바뀌고 이 표를 따라 갱신한다. 네이밍 **규칙·근거**는 [ADR-0016](../explanation/adr/0016-environment-service-domain-naming.md), 도구 **접근**(콘솔 링크·CLI·MCP)은 [tools.md](./tools.md) 참고.
...
| 도메인                   | 서비스                  | 환경       | 프로바이더 | 비고                                                                                                     |
| ------------------------ | ----------------------- | ---------- | ---------- | -------------------------------------------------------------------------------------------------------- |
| `career.bconnect.to`     | career (기술자 PWA)     | production | Vercel     |                                                                                                          |
| `bconnect.to`            | company (허브)          | production | Vercel     | `/`=업체 랜딩·`/career`=기술자 랜딩·`/one-click`. `/.well-known/assetlinks.json` 200 유지 (설치된 TWA용) |
...
| `dev.bconnect.to`        | company                 | dev        | Vercel     | apex 미러 — `dev` 브랜치 자동배포 (Vercel custom env)                                                    |
| `career.dev.bconnect.to` | career                  | dev        | Vercel     | `dev` 브랜치 자동배포 (Vercel custom env)                                                                |
| `plan.dev.bconnect.to`   | plan                    | dev        | Vercel     | `dev` 브랜치 자동배포. 2-레벨 서브도메인                                                                 |
| `api.dev.bconnect.to`    | api                     | dev        | Railway    | Railway `dev` 환경. Sentry env 태그 `dev`                                                                |
...
규칙: **`{service}.{env}.bconnect.to`** — production 은 `{env}` 생략 ([ADR-0016](../explanation/adr/0016-environment-service-domain-naming.md)). apex(`bconnect.to`)는 company 허브가 서빙. 새 환경·서비스는 같은 규칙으로 확장.
...
production = `main` 브랜치, dev = `dev` 브랜치 추적 (배포 절차: [deployment.md](../how-to/deployment.md)).
...
| dev (`career.dev.bconnect.to` 등) | `https://api.dev.bconnect.to` | Vercel `dev` custom env override, `NEXT_PUBLIC_API_MOCKING=disabled` → 실제 dev BE                                 |
| 로컬                              | MSW mock 기본                 | 실 BE 연동 시 `http://localhost:8080` fallback. 상세: [development-workflow.md](../how-to/development-workflow.md) |
...
| 서비스             | production                                                     | dev                                     | 프로바이더 | 선언 위치                                                                                                            |
| ------------------ | -------------------------------------------------------------- | --------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------- |
...
> DB 접속 문자열·password·TCP proxy host:port 등 **시크릿은 본 문서에 싣지 않는다** — `infra/` 변수(gitignored) 및 [Railway 콘솔](./tools.md) 참고.
