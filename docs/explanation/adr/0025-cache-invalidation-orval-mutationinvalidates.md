...
- **Status**: Proposed
- **Date**: 2026-07-04
- **Deciders**: @twjin03 (제안), @manamana32321 (CTO)
...
현재 이 invalidates는 **각 소비 파일이 수동 배선**한다 (현재 career 9개 파일에서 수동 배선). 현행 방식의 한계:
...
- 생성 훅 `onSuccess`에 `invalidateQueries`를 **자동 주입**한다.
...
- 진짜 낭비는 무효화 대상을 넓은 조건으로 잡을 경우에 발생하는데, 이는 rule `params` 를 사용해서 좁힐 수 있음. (즉, config 안에서 해결/감당 가능한 리스크)
