...
[PR #247](https://github.com/mortonCareer/bconnect/pull/247) (MSW migration) 코드 리뷰 중 envelope 정렬 문제 ([#252](https://github.com/mortonCareer/bconnect/issues/252)) 발견. 이를 출발점으로 spec 전반의 구조적 부채를 인지:
...
- **단점**:
  - 한 도메인 파일이 커질 수 있음 (현 최대: profiles 299줄 — 통념 안)

### Option 4: OpenAPI 3.0 유지 vs 3.1 업그레이드

3.1은 [JSON Schema Draft 2020-12](https://json-schema.org/) 호환. nullable, examples, webhooks 등 표현력 ↑. 하위 호환 깨지는 부분 있음 (e.g., `nullable: true` → `type: ['string', 'null']`).
...
**Option 3 (도메인 packing 14 파일) + Option 4 (3.1 업그레이드) + envelope wrap 통합** 채택.
...
- spec 변경 워크플로: [`how-to/development-workflow.md`](../../how-to/development-workflow.md) 참조
