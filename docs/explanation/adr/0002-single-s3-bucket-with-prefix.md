...
- 상태: 승인됨
- 날짜: 2026-04-17 (오프라인 합의), 2026-04-26 ([PR #227](https://github.com/mortonCareer/bconnect/pull/227) 머지)
- 담당자: @manamana32321 @fine-pine

## 개요

Sprint 2에서 파일 업로드 기능이 다수 entity에 걸쳐 도입됨. 대상은 프로필 이미지, 자격 증명, 게시글 첨부, 채팅 첨부, 동산보드판 저장소. 다음 요구가 충돌:

- 격리: 권한별로 다른 entity 파일을 분리하고 싶음. 프로필 = 공개, 자격 증명 = 본인+어드민, 채팅 = 참여자, 저장소 = 권한 멤버
...
### 옵션 1: 단일 버킷 + path prefix + CF Behavior 분기
...
- 운영 도구 단일 컨텍스트: CLI, 모니터링
- 도메인 단일로 CF cache hit ratio ↑
...
- 이미 별도 환경인 AWS 계정 분리와 중복. Morton은 단일 AWS 계정 + IAM role 분리로 환경 격리 → 버킷 분리는 redundant

## 결정사항

옵션 1 (단일 버킷 + path prefix + CF Behavior 분기) 채택.
...
- MVP 단계인 Morton 규모에서 운영 단순성이 격리 강도보다 우선
...
- 좋은 결과:
  - 신규 entity는 prefix만 정의해서 즉시 사용 가능. 예: 동산보드판 추가 카테고리
  - 운영 도구 / 알람 / 모니터링 단일 컨텍스트
  - CF 비용 / 관리 비용 최소화
- 나쁜 결과:
  - Signed Cookie scope 발급 로직 누락 시 cross-context leak 위험. 백엔드 권한 검증 코드 리뷰 강화 필요
  - blast radius: 한 버킷 장애 시 모든 파일 기능 영향. S3는 99.99% SLA이므로 수용 가능
- 중립적 결과:
...
- [#174](https://github.com/mortonCareer/bconnect/issues/174)
- [PR #227](https://github.com/mortonCareer/bconnect/pull/227)
