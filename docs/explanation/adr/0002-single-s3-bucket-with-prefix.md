...
Sprint 2에서 파일 업로드 기능(프로필 이미지, 자격 증빙, 게시글 첨부, 채팅 첨부, 동산보드판 저장소)이 다수 entity에 걸쳐 도입됨. 다음 요구가 충돌:

- **격리**: 권한별로 다른 entity 파일을 분리하고 싶음 (프로필 = 공개, 자격 증빙 = 본인+어드민, 채팅 = 참여자, 저장소 = 권한 멤버)
