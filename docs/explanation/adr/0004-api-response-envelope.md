...
- 상태: 승인됨
- 날짜: 2026-05-08 (백필 작성일. 결정 자체는 BE 초기 — 정확한 origin 추적 곤란)
- 담당자: @fine-pine (BE 초기 도입), @manamana32321 (spec 정렬)
...
- TanStack Query / orval-generated hooks이 응답 wrapping을 일관 처리 못 함

BE 초기 당시 이미 envelope 패턴이 코드에 존재했으나 spec까지 정렬되지 않아 FE가 추측으로 사용. [PR #266](https://github.com/mortonCareer/bconnect/pull/266)에서 spec까지 정렬.

## 선택지

### 옵션 1: REST 표준 (HTTP status + body)
...
- HTTP 표준 그대로
- HTTP 인프라 친화적: CDN, monitoring

단점

- 에러 body 형식이 endpoint마다 다를 위험
- FE가 status 검사 + body parse 둘 다 분기 필요
- 검증 실패 등 비즈니스 에러를 4xx로 표현할지 200+success:false로 표현할지 끝없는 논쟁

### 옵션 2: [JSON:API](https://jsonapi.org/)
...
### 옵션 3: 단순 envelope `{success, data, error}`
...
- HTTP status는 인프라용, biz 의미는 envelope 안. 분리 명확
...
### 옵션 4: GraphQL-like errors[]

`{ data: T, errors: [{ code, message, path }, ...] }`. 부분 성공 표현 가능.

장점

- 부분 성공 표현 가능. 한 응답에 여러 entity, 일부만 실패

단점

- REST endpoint는 보통 단일 entity → 부분 성공 표현이 거의 무용

## 결정사항

옵션 3 (단순 envelope) 채택.

Spec 정의는 구 `_shared.yaml`, 현재 BE springdoc 산출:
...
- FE 에러 핸들링 일관 패턴 강제
- orval codegen + TanStack Query 통합이 envelope 패턴에 자연스러움
- HTTP status는 인프라 용도로 유지하되 비즈니스 의미는 envelope에서 결정. 인프라는 CDN cache, monitoring.
...
- 나쁜 결과:
  ...
  - PUT/DELETE 일부인 void 응답 wrap 안 함 → ExtractData fallback 처리 필요. codegen 복잡도 약간 ↑
- 중립적 결과:
...
- [CLAUDE.md](../../../.claude/CLAUDE.md)에 "API Response Wrapper" 패턴 명시
- 후속: void 응답 unwrap 패턴 검증. orval generated hooks의 `data` 타입이 정확히 `void`로 떨어지는지 spot check 필요

## 참조

- [PR #266](https://github.com/mortonCareer/bconnect/pull/266) : spec 정렬
- `_shared.yaml` : 구 손-spec envelope schema. [ADR-0024](./0024-orval-consumes-be-springdoc-spec.md) 로 폐기, 현재 BE springdoc 산출
