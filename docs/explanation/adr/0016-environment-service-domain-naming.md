# ADR-0016: 환경·서비스 도메인 네이밍 — `{service}.{env}.bconnect.to`
...
production 도메인은 이미 자리잡았다: career=apex `bconnect.to`, plan=`plan.bconnect.to`, api=`api.bconnect.to`. dev 도메인도 이와 **연속적인 규칙**으로 정의해, 사람이 URL만 보고 "어느 서비스 / 어느 환경"인지 즉시 알 수 있어야 한다.

## Options

### Option 1: `{service}.{env}.bconnect.to` (production 은 env 생략)

production 은 `{service}.bconnect.to`, 비-production 환경은 service 와 apex 사이에 환경 라벨을 삽입. career 는 apex 서비스라 `{service}` 가 빈 형태.

- **장점**: production 도메인 불변(기존 유지), 환경 라벨이 apex 바로 앞이라 "환경"이 도메인 계층에서 명확, 서비스·환경 추가 시 규칙 그대로 확장
- **단점**: career(apex)는 `{service}` 가 비어 dev 가 `dev.bconnect.to` — 패턴의 미세한 예외. `plan.dev.bconnect.to` 는 2-레벨 서브도메인이라 와일드카드 TLS/CORS 에 주의 필요

### Option 2: `{env}-{service}.bconnect.to`

환경을 prefix 로 (`dev-api.bconnect.to`, `dev-plan.bconnect.to`, career dev = `dev.bconnect.to`).

- **장점**: 모두 1-레벨 서브도메인 → `*.bconnect.to` 와일드카드 하나로 TLS/CORS 커버
- **단점**: production 과 형태 단절(`api.bconnect.to` ↔ `dev-api.bconnect.to`), 환경·서비스가 하이픈으로 평면화돼 계층 의미 약함, 서비스명에 하이픈 들어가면 모호

### Option 3: 자동생성 URL 유지 (커스텀 도메인 없음)

- **장점**: 작업 0
- **단점**: 위 Context 의 비일관·결합·확장성 문제 그대로
...
production 도메인을 손대지 않는 연속성과, 환경이 apex 앞에 오는 계층 명확성을 우선시했다. Option 2 의 와일드카드 단순함보다 production-연속성·가독성을 택했고, 그 대가로 2-레벨 서브도메인(`plan.dev.bconnect.to`)에 대한 와일드카드 처리 부담을 받아들인다.
...
- 현재 도메인 현황표(이 규칙의 적용 결과): [도메인 현황](../../reference/domains.md)
