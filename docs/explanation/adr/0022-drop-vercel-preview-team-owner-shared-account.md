...
- **Related**: [#571](https://github.com/mortonCareer/bconnect/issues/571) (구현 추적), [ADR-0006](./0006-dev-as-staging.md), [ADR-0016](./0016-environment-service-domain-naming.md), [ADR-0018](./0018-vercel-native-skip-unaffected-projects.md), [git-workflow.md](../../how-to/git-workflow.md)

## Context

BConnect 레포(`mortonCareer/bconnect`)는 **private** 이고, Vercel 은 Team → Projects(1:N) 구조로 career·plan 2개 프로젝트를 한 Team 아래 둔다. PR 마다 자동 생성되는 **프리뷰 배포**의 본래 목적은 디자이너 UI 검수(+ CTO 기능 테스트, CEO 최종 QA)였고, 이는 [git-workflow.md](../../how-to/git-workflow.md) 의 QA 사이클이 전제로 깔고 있다.
...
- **단점**: PR 단위 즉시 시각 검수 상실(회귀가 스프린트 QA 시점에야 발견될 수 있음)
...
받아들인 트레이드오프는 **PR 단위 프리뷰의 상실**이다. UI 검수는 PR 단위 → **스프린트 단위 dev 환경 QA** 로 옮긴다. [레포 Public 전환(Option 3)은 소스 공개 불가로 배제](#options).
...
- **후속**: Team Owner 이전(개인→공용)과 PR 프리뷰·체크 비활성화 실행은 [#571](https://github.com/mortonCareer/bconnect/issues/571).
- 동기화된 워크플로 문서: [git-workflow.md](../../how-to/git-workflow.md)(QA 서술을 스프린트 단위 dev 로 정합).
