...
- **plan**(업체 앱): 오른쪽에서 슬라이드로 나오는 서랍(패널) 안에 같은 화면을 보여준다.
...
1. **껍데기가 화면에 박혀 있었다.** 공용 `ProfileView`는 plan의 서랍 껍데기(`PanelShell` — 서랍 위치 좌표, Esc로 닫기, 오른쪽 화살표 닫기 버튼)를 자기 안에 통째로 박아 두었다. career의 풀페이지(TopBar + 아래 탭)는 이 서랍 껍데기를 쓸 수가 없다.
...
여기에 큰 제약이 하나 더 붙는다: **로그인·회원가입을 뺀 career 화면의 약 90%가 결국 plan 서랍에도 들어간다.** 즉 `@bconnect/features`는 곧 제품 화면 거의 전부의 원본 저장소가 된다. 지금 정하는 방식이 **앞으로 수십 개 화면을 옮길 때 따라 할 본보기**가 된다. 한 번의 선택이 30번 곱해진다.
...
plan   ─ 오른쪽 서랍 (PanelShell)       남 화면 ─ 가려진 정보, 동료추가·메시지
...
┌─────────┴──────────┐ ┌──────────┴─────────┐  ┌──────────────┴──────────────┐
│ career 내 화면      │ │ career 남 화면      │  │ plan 서랍                   │
...
└────────────────────┘ └────────────────────┘  └─────────────────────────────┘
   /profile (내 화면)     /profile/[memberId]      plan 오른쪽 서랍
```
...
apps/plan/src/app/(main)/@panel/profile/[profileId]/
  page.tsx                 ▲ 데이터를 페이지에서 미리 가져와 내려줌. 껍데기 안 끼움(기본 서랍)
...
1. **껍데기**: `ChatView`/`ProfileView`의 껍데기 끼우기 타입을 그대로 복사한다. career는 한 번 만들어 둔 `careerShell(onBack?)`을 넘기고, plan은 안 넘긴다(그러면 기본 서랍). 이 타입 모양은 다시 짜지 말고 그대로 쓴다.
...
- **나쁜 점**: 앱 페이지에 버튼 JSX와 데이터 호출 코드가 남는다(어댑터로 줄지만 0은 아니다). prop 타입이 길어진다. 데이터를 위로 올리느라 plan 서랍 페이지도 손봐야 한다(동작은 그대로지만 고친 줄은 있다).
