...
QA 중 발견된 문제는 다음 기준으로 분류 (레이블은 [labels.md](../reference/labels.md#버그-레이블) 매핑):

| 증상                           | 범위          | 담당      | 예시                                   |
| ------------------------------ | ------------- | --------- | -------------------------------------- |
| 스펙 자체가 부족/잘못됨        | `📋 api-spec` | 둘이 논의 | 필요한 필드가 API 응답에 없음          |
| 응답이 스펙과 다름             | `⚙️ be`       | CEO       | `name` 필드가 null인데 스펙은 required |
| 스펙대로 왔는데 화면 처리 안됨 | `💻 fe`       | CTO       | 에러 응답이 왔는데 에러 메시지 미표시  |
