...
class PostServiceTest {
    ...
    void list_success() {
        ...
        // when
        val firstPage = postService.list(new CursorLimit(null, 2, null));
        val defaultPage = postService.list(new CursorLimit(null, null, null));
    ...
    void create_success() {
        ...
        val postId = postService.create(
                ...
                PostFactory.createCommand(task.getId(), attachment.getId())
        );
    ...
    void create_fail_C004() {
        ...
        // when & then
        assertCodeException(() -> postService.create(UserFactory.domain(member.getId(), Role.CAREER),
                PostFactory.createCommand(task.getId(), attachment.getId())))
    ...
    void create_fail_C005() {
        ...
        // when & then
        assertCodeException(() -> postService.create(UserFactory.domain(member.getId(), Role.CAREER),
                PostFactory.createCommand(MISSING_ID, attachment.getId())))
                .hasExceptionCode(CommonExceptionCode.NOT_FOUND);
    }

    @Test
    @DisplayName("update - 내 게시글일 때 수정하면 내용이 갱신되고 기존 첨부가 해제되며 새 첨부가 연결된다")
    void update_success() {
        // given
        val member = memberRepository.save(MemberFactory.entity("member1", "01000001001", Role.CAREER));
        val task = taskRepository.save(TaskFactory.entity(member.getId()));
        ...
        postService.update(
                UserFactory.domain(member.getId(), Role.CAREER),
                post.getId(),
                PostFactory.updateCommand(task.getId(), second.getId())
        );
    ...
    void update_fail_C004_post() {
        ...
        // when & then
        assertCodeException(() -> postService.update(
                UserFactory.domain(other.getId(), Role.CAREER),
                post.getId(),
                new UpdatePost(null, List.of(), "updated content")
        )).hasExceptionCode(CommonExceptionCode.FORBIDDEN);
    }
    ...
    void update_fail_C004_task() {
        ...
        // when & then
        assertCodeException(() -> postService.update(UserFactory.domain(member.getId(), Role.CAREER),
                post.getId(), PostFactory.updateCommand(task.getId(), attachment.getId())))
                .hasExceptionCode(CommonExceptionCode.FORBIDDEN);
    }
    ...
    void update_fail_C005_task() {
        ...
        // when & then
        assertCodeException(() -> postService.update(UserFactory.domain(member.getId(), Role.CAREER),
                post.getId(), PostFactory.updateCommand(MISSING_ID, attachment.getId())))
                .hasExceptionCode(CommonExceptionCode.NOT_FOUND);
    }

    @Test
    @DisplayName("delete - 내 게시글일 때 삭제하면 게시글이 삭제되고 첨부 참조가 해제되며 없는 게시글은 무시된다")
    void delete_success() {
        // given
        val member = memberRepository.save(MemberFactory.entity("member1", "01000001001", Role.CAREER));
        val post = postRepository.save(PostFactory.entity(member.getId(), null));
        ...
        // when
        postService.delete(UserFactory.domain(member.getId(), Role.CAREER), post.getId());
        postService.delete(UserFactory.domain(member.getId(), Role.CAREER), MISSING_ID);
    ...
    void delete_fail_C004() {
        ...
        // when & then
        assertCodeException(() -> postService.delete(UserFactory.domain(other.getId(), Role.CAREER), post.getId()))
                .hasExceptionCode(CommonExceptionCode.FORBIDDEN);
    }
}
