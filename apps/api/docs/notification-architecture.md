...
```mermaid
sequenceDiagram
    participant Pub as MessageSocketService · MemberService
    ...
    Pub->>Listener: SocketMessageSentEvent · MemberRegisteredEvent
...
| 단계 | 처리 | DB | Push |
|---|---|---|---|
| trigger | 채팅 메시지 저장 후 `SocketMessageSentEvent`, 회원가입 후 `MemberRegisteredEvent` 발행 | messages · members | - |
...
| COWORKER_REQUESTED | MEMBER | COWORKER_REQUEST | `%s 님으로부터 동료 요청을 제안받았습니다` | ⬜ 미배선 |
| OFFER_RECEIVED | COMPANY | OFFER | `%s으로부터 섭외 요청을 제안받았습니다` | ⬜ 미배선 |
