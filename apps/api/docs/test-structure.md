...
### 테스트 메서드
- 메서드 핵심 로직만 검증합니다
  - 인자 정규화(null 가드, null 요소 제거, 중복 제거 등)는 검증하지 않습니다
  - 멱등 처리(미존재 대상 무시), 빈 결과, 커서 다음 페이지는 검증하지 않습니다
- 필드는 **PK · FK · unique · 해당 메서드가 바꾸는 필드**만 단언합니다
  - 변경되지 않음을 확인하는 단언은 두지 않습니다
- 데이터 객체는 `// given`에서 개별 `val`로 생성합니다
