...
export function CoworkerCard({ profileId }: CoworkerCardProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex animate-pulse items-center gap-3 px-4 py-3">
        <div className="h-12 w-12 shrink-0 rounded-full bg-bconnect-gray-200" />
        <div className="flex flex-1 flex-col gap-2">
          <div className="h-4 w-24 rounded bg-bconnect-gray-200" />
          <div className="h-3 w-40 rounded bg-bconnect-gray-200" />
        </div>
      </div>
    )
  }
  ...
  return (
    <button
      ...
      className="flex w-full cursor-pointer items-center gap-3 px-4 py-3 transition-colors active:bg-bconnect-gray-100"
    >
      {/* 프로필 이미지 */}
      ...
      <div className="flex min-w-0 flex-1 flex-col items-start gap-0.5">
        ...
        {intro && (
          <p className="line-clamp-2 text-left text-r-14 text-bconnect-gray-500">{intro}</p>
        )}
      </div>
