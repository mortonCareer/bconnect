...
export const MOCK_USER: SidebarUser = { name: '이송목님', role: '자담디자인' }
export const MOCK_NOTIFICATION_COUNT = 4
export const MOCK_MESSAGE_COUNT = 4
...
export const MOCK_PROJECT_MENU: SidebarProjectMenuItem[] = [
  { label: '공정표', slug: 'schedule' },
  { label: '모집 관리', slug: 'recruit' },
  { label: '문서 저장소', slug: 'docs' },
]

export const MOCK_TASK_ROWS: TaskRow[] = TASKS.map((t) => ({
  id: t.id,
  category: t.category,
  status: t.status,
  assignee: t.assignee,
...
export const MOCK_GANTT_TASKS: GanttTask[] = TASKS.map((t) => ({
  id: t.id,
  name: t.ganttName,
  category: t.category,
