...
- 패널: `_components/panel/PanelHost` 가 `?panel=` 을 읽어 dispatch — `profile/[id]`(+`/coworkers`,`/recommendations`), `crawled/[id]`, `messages`·`messages/[id]`(채팅), `notifications`, `task/new`·`task/[id]`. 각 `Panel*` 컴포넌트가 데이터 prep, 공유 뷰는 `@bconnect/features`
